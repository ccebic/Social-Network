...
const delModal = document.querySelector('.custom-modal');
        ...
        document.querySelector('#allPostsWrapper').innerHTML = `
            <div class="single-post" data-post_id="${post.id}">
                ...
                <form>
                    ...
                    <button onclick="commentPostSubmit(event)" class="comment-btn-validate">Comment</button>
                </form>
                <p class="rule">Minimalan broj karaktera je 1</p>
                </div>
            </div>
        ...
        async function getPostUser() {
            ...
            if (comments.length > 0) {
                comments.forEach(comment => {
                    comment_html += `<div class="single-comment">
                                        ${comment.content}
                                        <br>
                                        <span><b>Autor: </b>${comment.usercomment}</span>
                                     </div>
                                        `;
                });
            }
...
const commentPostSubmit = e => {
    async function setComment() {
        ...
        main_post_el.querySelector('.post-comments').innerHTML += `<div class="single-comment">
                                                                      ${comment_value} 
                                                                      <br>
                                                                    <span><b>Autor: </b>${current_user.username}</span>
                                                            </div>`
        ;
        ...
        comment.usercomment = current_user.username;
...
const commentPost = btn => {
    ...
    //*Moj nacin bolji zbog toggla
    let showComment = btn.closest('.single-post');
    let comment = showComment.querySelector('.post-comments');
    comment.classList.toggle('active');
    commentValidation();
};

function commentValidation() {
    let postComments = document.querySelectorAll('.post-comments');
    postComments.forEach(postComment => {
        let buttonValidate = postComment.querySelector('form .comment-btn-validate');
        let formVdaliteComment = postComment.querySelector('form input');
        let rule = postComment.querySelector('.rule');
        let postComments = postComment.querySelector('.post-comments');
        buttonValidate.disabled = true;

        formVdaliteComment.addEventListener('input', form => {
            let formLength = form.target.value.length;
            let formValue = form.target.value;
            if (formLength > 0) {
